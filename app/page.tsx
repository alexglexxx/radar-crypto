...
function getAsistente(symbol: string, sel: any, history: any[]) {
  ...
  const firstScore = last.length > 0? last[0].score : score
  const lastScore = last.length > 0? last[last.length - 1].score : score
  const subiendo = lastScore >= firstScore
  const plano = Math.abs(lastScore - firstScore) < 5
  const precio = Number(sel.price) || 0
  const entrada = precio.toFixed(2)
  const sl = (precio * 0.97).toFixed(2)
  const tp = (precio * 1.05).toFixed(2)
  ...
  if (score >= 80 && subiendo) {
    return {
      ...
      porQue: `Score ${score} y viene subiendo de ${firstScore} a ${lastScore}. Precio + volumen + noticias alineados. Es el patron que esperamos toda la semana.`,
      paso: `1. Entrada: $${entrada}\n2. SL: $${sl} (-3%)\n3. TP: $${tp} (+5%)\n4. Solo 10% de tu capital.`,
      mente: 'No corras. Si no te da entrada en 10 min, no era tu taxi. Vendra otro.'
    }
  }
  if (score >= 70) {
    return {
      estado: `${symbol} FUERTE PERO PLANO`,
      color: '#eab308',
      queHacer: 'ESPERA CONFIRMACION',
      porQue: `Score ${score} bueno pero plano. Ya subio y ahora duda. Si compras aqui te puedes quedar atrapado arriba.`,
      paso: `1. No compres aun.\n2. Espera 2-3 barras mas.\n3. Si sube a 85+ y precio rompe arriba, entras.\n4. Si baja a 60, te ahorraste perdida.`,
      mente: 'Aqui el novato pierde por FOMO. Tu esperas confirmacion. La paciencia paga.'
    }
  }
  if (score >= 45) {
    return {
      estado: `${symbol} EN AMARILLO`,
      color: '#eab308',
      queHacer: 'NO TOQUES NADA',
      porQue: `Score ${score} es tierra de nadie. Como tu ETH en 50 de la foto. Ni sube ni baja. No hay operacion clara.`,
      paso: `1. Cierra Bitso.\n2. Pon alarma en 75+.\n3. Mira las otras 3 monedas, la que este verde es tu oportunidad.`,
      mente: 'Tu cerebro dira "haz algo". Ese es el enemigo. En amarillo, hacer nada ES hacer algo bien.'
    }
  }
  return {
    ...
    porQue: `Score ${score} bajo = precio cayendo o noticia mala. Comprar aqui pensando "esta barato" te sale mas barato manana.`,
    paso: `1. Si estas dentro, revisa si ya te saco el SL.\n2. Si no estas, quedate fuera.\n3. Anota en diario por que bajo.`,
    mente: 'La mejor operacion del dia a veces es no operar. Hoy tu ganancia es no perder -3%.'
  }
}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1.2fr 0.9fr', gap: 8 }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            ...
            <div style={{ background: '#101010', border: '1px solid #222', borderRadius: 12, padding: 12 }}>
              ...
              <div style={{ display: 'flex', gap: 6 }}>
                <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between', fontSize: 8, color: '#555', height: 90, textAlign: 'right' }}><span>${maxPrice.toFixed(0)}</span><span>${minPrice.toFixed(0)}</span></div>
                <div style={{ flex: 1, position: 'relative', height: 90, borderLeft: '1px solid #333', borderBottom: '1px solid #333'[STRIPPED 34 bytes]"100%" height="100%" style={{ overflow: 'visible' }}>
            ...
            {asistente && (
              <div>
                ...
                <div style={{ padding: 12, display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <div>
                    <div style={{ fontSize: 9, color: asistente.color, fontWeight: 800 }}>QUE HACER AHORA</div>
                  ...
                  <div style={{ background: '#000', border: '1px solid #222', borderRadius: 8, padding: 10 }}>
                    <div style={{ fontSize: 9, color: '#888', fontWeight: 700 }}>POR QUE</div>
                    <div style={{ fontSize: 11, color: '#ccc', marginTop: 6, lineHeight: 1.4 }}>{asistente.porQue}</div>
                  </div>
                  ...
                  <div style={{ background: '#1a1a0a', border: '1px dashed #444', borderRadius: 8, padding: 10 }}>
                    ...
                    <div style={{ fontSize: 11, color: '#aaa', marginTop: 6, lineHeight: 1.4, fontStyle: 'italic' }}>{asistente.mente}</div>
                  </div>
                </div>
              </div>
            )}
