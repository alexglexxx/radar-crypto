...
export default function Page(){
  ...
  function buildAsist(){
    ...
    if(sc >= 80 && sube){
      setAsist({ t: selected + ' EXPLOSIVO', c: '#22c55e', qh: 'PREPARA ENTRADA AHORA', pq: 'Score ' + sc + ' subiendo de ' + first + ' a ' + last + '. Precio + volumen alineados. Patron pro.', paso: 'Entrada $' + ent + ' | SL $' + sl + ' -3% | TP $' + tp + ' +5% | Solo 10% capital', mente: 'No corras. Si no da entrada en 10 min, vendra otro taxi.' })
    } else if(sc >= 70){
      setAsist({ t: selected + ' FUERTE PERO PLANO', c: '#eab308', qh: 'ESPERA CONFIRMACION', pq: 'Score ' + sc + ' bueno pero plano. Ya subio y duda. Si compras aqui te quedas atrapado.', paso: 'No compres aun | Espera 2-3 barras | Si sube a 85+ entras | Si baja a 60 se cancela', mente: 'Aqui el novato pierde por FOMO. Tu esperas. La paciencia paga.' })
    } else if(sc >= 45){
      setAsist({ t: selected + ' EN AMARILLO', c: '#eab308', qh: 'NO TOQUES NADA', pq: 'Score ' + sc + ' tierra de nadie. Ni sube ni baja. No hay operacion clara.', paso: 'Cierra Bitso | Alarma en 75+ | Mira otras 3 monedas', mente: 'En amarillo, hacer nada ES hacer algo bien.' })
    } else {
      setAsist({ t: selected + ' NO SE TOCA', c: '#ef4444', qh: 'PROTEGE TU CAPITAL', pq: 'Score ' + sc + ' bajo = cuchillo cayendo. No compres barato.', paso: 'Revisa SL | Quedate fuera | Anota por que bajo', mente: 'La mejor operacion a veces es no operar.' })
    }
  }
      ...
      <div style={{maxWidth:980, margin:'0 auto'}}>
        ...
        <div style={{display:'grid', gridTemplateColumns:'1.2fr 0.9fr', gap:8}}>
          <div style={{display:'flex', flexDirection:'column', gap:8}}>
            ...
            <div style={{background:'#101010', border:'1px solid #222', borderRadius:12, padding:12}}>
              <div style={{fontSize:11, fontWeight:700, marginBottom:6}}>PRECIO {selected} 6H</div>
              <div style={{flex:1, position:'relative', height:90, borderLeft:'1px solid #333', borderBottom:'1px solid #333'}}>
            ...
            {asist && (
              <div>
                ...
                <div style={{padding:12, display:'flex', flexDirection:'column', gap:12}}>
                  ...
                  <div style={{background:'#000', border:'1px solid #222', borderRadius:8, padding:10}}><div style={{fontSize:9, color:'#888', fontWeight:700}}>POR QUE</div><div style={{fontSize:11, color:'#ccc', marginTop:6}}>{asist.pq}</div></div>
                  <div style={{background:'#111', border:'1px solid #333', borderRadius:8, padding:10}}><div style={{fontSize:9, color:'#4ade80', fontWeight:700}}>PASO A PASO</div><div style={{fontSize:11, color:'#fff', marginTop:6}}>{asist.paso}</div></div>
